...
#사용하는 변수들
idA, moneyA, preA, slitA, timeA, give, ID, TIME = [], [], [], [], [], 0, 0, 0
...
while True: #유저들 데이터를 읽음 데이터 형식 : 유저ID,가지고 있는 돈,돈받은 시간
    ...
    slitA.append(int(line[3]))
with open("member.txt", "r") as w:
    while True:
        line = w.readline()
        if not line: break
        preA.append(line)#아 귀찮아
...
async def on_message(message):
    if message.content == "/코인받기":
        ...
        if ID in idA:
            ...
            for i in range(0,len(idA),1):
                f.write(str(idA[i]) + "," + str(moneyA[i]) + "," + str(timeA[i]) + "," + str(0) + "\n")
            f.close()
        elif not ID in idA:
            idA.append(ID)
            moneyA.append(give)
    ...
    if message.content.startswith("/도박"):
        ...
        if message.author.id in adminlist: #관리자라면 테슽 할것도 있고 하니 돈 많이 줌
            give = 999
        if slitA[idA.index(ID)] == "1":
            give = random.randrange(1, 11)
            moneygive = random.randrange(5, 21)
            if give == 1 or give == 2:
                embed = discord.Embed(title="도박 실패..", description="도박을 성공했으면 [ " + str(
                    moneygive) + "배 ] 의 돈을 얻을 수 있었는데 실패했네요.. \n \n 현재 스카이코인 • " + str(moneyA[idA.index(ID)]) + " 코인",
                                      timestamp=message.created_at, colour=discord.Colour.dark_red())
                embed.set_footer(text="도박권을 사용했을 때의 도박 확률은 80 : 20 이에요!")
                await message.channel.send(embed=embed)
            elif give <= 3:
                moneyA[idA.index(ID)] += give * msg[1]
                embed = discord.Embed(title="도박 성공!", description="도박을 성공하여 [ " + str(
                    moneygive) + "배 ] 의 돈을 얻었어요! \n \n 현재 스카이코인 • " + str(
    ...
    if message.content == "/올인":
        ...
        if not ID in idA or moneyA[idA.index(ID)] <= 0: #만약 돈이 부족하면
            ...
            await message.channel.send(embed=embed)
            raise ValueError
        if ID in preA:#프리미엄은 올인 못하게 하기(ㅈㄴ 귀찮음)
            await message.channel.send("프리미엄 유저는 /올인 을 사용할 수 없습니다!")
            pas = True
        give = random.randrange(2,10)
        await asyncio.sleep(1)
        if give % 2 == 0 and pas == False:
            moneyA[idA.index(ID)]*= give
...
client.run("NzM2MTM1NjcyODQyNDIwMzI2.XxqZxQ.DUq7pCmWrKccbocWksZGc7Hw-m0")
